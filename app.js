...
async function loadMisFaltas() {
  ...
  box.innerHTML = '<h4>Mis justificaciones</h4>' + list.map(b => {
    ...
    return `<div class="list-item">
      ...
      ${(b.archivos||[]).map(a => '<a class="link" href="' + esc(a.url) + '" target="_blank">📎 ' + esc(a.name) + '</a>').join(' ')}
      </div></div>`;
  }).join('');
}
...
window.histDay = async function(fecha) {
  ...
  if (a && a.fotoURL) inner += ' <a class="link" href="' + esc(a.fotoURL) + '" target="_blank">📷 Ver evidencia</a>';
  $('#hist-detail').innerHTML = '<div class="notice">' + inner + '</div>';
};

/* ---------- MIS GUARDIAS (ASPIRANTES) ---------- */
async function vMisGuardias() {
  const g = await Store.listGuards({ uid: ME.id });
  const hoy = todayStr();
  const grupos = {
    proximas: g.filter(x => x.fecha >= hoy && x.estado === 'pendiente'),
    cumplidas: g.filter(x => x.estado === 'cumplida'),
    justificadas: g.filter(x => x.estado === 'justificada'),
    no_cumplidas: g.filter(x => x.estado === 'no_cumplida')
  };
  const row = x => `<div class="list-item"><span class="chip ${ESTADO_STYLE[x.estado][2]}">${ESTADO_STYLE[x.estado][0]} ${ESTADO_STYLE[x.estado][1]}</span> <strong>${fechaNice(x.fecha)}</strong></div>`;
  $('#view-guardias').innerHTML = `
    <div class="panel">
      <h3>🌙 Mis guardias nocturnas</h3>
      <h4>Próximas</h4>${grupos.proximas.length ? grupos.proximas.map(row).join('') : '<p class="hint">Sin guardias programadas.</p>'}
      <h4>Cumplidas (${grupos.cumplidas.length})</h4>${grupos.cumplidas.length ? grupos.cumplidas.map(row).join('') : '<p class="hint">—</p>'}
      <h4>Justificadas (${grupos.justificadas.length})</h4>${grupos.justificadas.length ? grupos.justificadas.map(row).join('') : '<p class="hint">—</p>'}
      <h4>No cumplidas (${grupos.no_cumplidas.length})</h4>${grupos.no_cumplidas.length ? grupos.no_cumplidas.map(row).join('') : '<p class="hint">—</p>'}
    </div>`;
}
  ...
  $('#admin-view').innerHTML = `
    ...
    <div class="two-col">
      <div class="panel">
        ...
        ${topFaltas.map((r,i) => `<div class="list-item"><b>#${i+1}</b> ${esc(fullName(r.user))} <span class="chip st-bad">${r.just + r.injust}</span> <small>(${r.just} just. / ${r.injust} injust.)</small></div>`).join('') || '<p class="hint">Sin datos.</p>'}
      </div>
      <div class="panel">
        <h4>⏱ Más tardanzas</h4>
        ${topTard.map((r,i) => `<div class="list-item"><b>#${i+1}</b> ${esc(fullName(r.user))} <span class="chip st-late">${r.tard}</span></div>`).join('') || '<p class="hint">Sin datos.</p>'}
      </div>
    </div>`;
}
...
async function renderAdminCal() {
  ...
  if (state.calMode === 'dia') {
    ...
    const b = absByFecha[f] || [];
    const ok = a.filter(x => x.tipo === 'asistencia');
    const late = a.filter(x => x.tipo === 'tardanza');
    const just = b.filter(x => x.estado !== 'rechazada');
    const justOk = just.filter(x => x.estado === 'aprobada');
    const justPen = just.filter(x => x.estado === 'pendiente');
    const justSet = new Set(just.map(x => x.uid));
    const injust = activos.filter(u => !a.some(x => x.uid === u.id) && !justSet.has(u.id));
    const bloque = (titulo, cls, arr, render) => `
      <div class="panel ${cls}"><h4>${titulo} (${arr.length})</h4>
      ${arr.length ? arr.map(render).join('') : '<p class="hint">Nadie.</p>'}</div>`;
    det.innerHTML = `<h3 class="view-title">📅 ${fechaNice(f)}</h3><div class="two-col">
      ${bloque('✅ Asistieron', 'p-green', ok, x => `<div class="list-item">${esc(uName(x.uid))} <small>${esc(x.hora)}</small> ${x.fotoURL ? '<a class="link" href="' + esc(x.fotoURL) + '" target="_blank">📷</a>' : ''}</div>`)}
      ${bloque('🟠 Llegaron tarde', 'p-orange', late, x => `<div class="list-item">${esc(uName(x.uid))} <small>${esc(x.hora)}</small> ${x.fotoURL ? '<a class="link" href="' + esc(x.fotoURL) + '" target="_blank">📷</a>' : ''}</div>`)}
      ${bloque('🟡 Faltaron justificadamente', 'p-yellow', just, x => `<div class="list-item">${esc(uName(x.uid))} <small>(${esc(x.estado)})</small> — ${esc(x.motivo || '')}</div>`)}
      ${bloque('🔴 Faltaron injustificadamente', 'p-red', f <= todayStr() ? injust : [], x => `<div class="list-item">${esc(fullName(x))}</div>`)}
    </div>`;
...
async function aGuardias() {
  ...
  const aspirantes = users.filter(u => u.categoria === 'aspirante' && u.activo !== false);
  const uName = id => { const u = users.find(x => x.id === id); return u ? fullName(u) : id; };
  const sorted = [...guards].sort((a,b) => b.fecha.localeCompare(a.fecha));
  $('#admin-view').innerHTML = `
    <h3 class="view-title">🌙 Control de guardias nocturnas</h3>
    <div class="panel">
      <h4>Asignar guardia</h4>
      <form class="inline-form" onsubmit="return submitGuard(event)">
        <select id="g-uid" required>${aspirantes.map(a => '<option value="' + a.id + '">' + esc(fullName(a)) + '</option>').join('')}</select>
        <input type="date" id="g-fecha" value="${todayStr()}" required>
    ...
    <div class="panel">
      ...
      </tr>`).join('') || '<tr><td colspan="4" class="hint">Sin guardias registradas.</td></tr>'}
      </tbody></table>
    </div>`;
}

window.submitGuard = async function(e) {
  e.preventDefault();
  await Store.addGuard({ uid: $('#g-uid').value, fecha: $('#g-fecha').value });
  await audit('ASIGNÓ GUARDIA', ME.username + ' asignó guardia del ' + $('#g-fecha').value);
  toast('🌙 Guardia asignada.');
  aGuardias();
  return false;
};
window.setGuardEstado = async function(id, estado) {
  await Store.updateGuard(id, { estado });
  await audit('ACTUALIZÓ GUARDIA', ME.username + ' → ' + id + ' = ' + estado);
  toast('Guardia actualizada.');
};
window.delGuard = async function(id) {
...
async function aRankings() {
  ...
  if (rkTab === 'faltas') {
    ...
    const tbl = (arr, fn, lbl) => `<div class="panel"><h4>${lbl}</h4>
      ${arr.map((r,i) => `<div class="list-item"><b>${medal(i)}</b> ${esc(fullName(r.user))} <span class="chip st-bad">${fn(r)}</span></div>`).join('') || '<p class="hint">Sin datos.</p>'}</div>`;
    ...
    body = `<div class="panel"><h4>⏱ Ranking de tardanzas</h4>
      ${tard.map((r,i) => `<div class="list-item"><b>${medal(i)}</b> ${esc(fullName(r.user))} <span class="chip st-late">${r.tard} tardanza(s)</span> · 🟢 ${r.asist} asistencias</div>`).join('') || '<p class="hint">Sin datos.</p>'}</div>`;
  }
  $('#admin-view').innerHTML = `
